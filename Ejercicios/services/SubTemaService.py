...
class SubTemaService:
    ...
    async def getSubTemasByTemaId(self, tema_id: str):
        try:
            ...
            for subtema in subtemas:
                ...
                subtema["tema_id"] = str(subtema.get("tema_id", ""))
                cont += 1
            print("Subtemas encontrados:", subtemas)
    ...
    async def generateSubTemasIdeas(self, tema_str:str):
        try:
    ...
    async def getVideosRecommendatioForSubTema(self, subtema_id:str):
        try:
            subtema = await self.subtema_repository.getSubTemaById(subtema_id)
            if subtema is None:
                return {"error": "El subtema no existe"}
            tema = await self.tema_repository.getTemaById(subtema.get("tema_id", ""))
            if tema is None:
                return {"error": "El tema del subtema no existe"}
            descripciones = tema.get("descripciones", [])
            if not descripciones:
                return {"error": "No hay descripciones disponibles para el tema del subtema"}
            resultado = await self.ia_service.generateVideoSImilarity(descripciones, subtema.get("titulo", ""))
            return resultado
        except Exception as e:
            return {"error": str(e)}
    ...
    async def addVideosToSubTema(self, subtema_id: str, videos_urls: ListYoutubeTemasCreation):
        try:
            ...
            if subtema is None:
                return {"error": "El subtema no existe"}
            for video_url in videos_urls.urls:
                if not video_url.startswith("https://www.youtube.com/watch?v="):
                    return {"error": f"URL de video inválida: {video_url}"}
                added = await self.subtema_repository.addVideoToSubTema(subtema_id, video_url)
                if not added:
                    return {"error": f"No se pudo agregar el video: {video_url}"}
